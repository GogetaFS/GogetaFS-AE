...
SWAP_TIME=1
SWAP_MEM_SIZES=( $((1024 * 1024 * 128)) $((1024 * 1024 * 256)) $((1024 * 1024 * 384)) $((1024 * 1024 * 512)) )
...
TABLE_NAME="$ABS_PATH/performance-comparison-table-continuous-hybrid"
                ...
                for job in "${NUM_JOBS[@]}"; do
                    for swap_mem in "${SWAP_MEM_SIZES[@]}"; do
                        ...
                        if ((dup_rate == 100)); then
                            ...
                            _=$(sudo fio -directory=/mnt/pmem0/first -fallocate=none -direct=1 -iodepth 1 -rw=write -ioengine=sync -bs=2M -thread -numjobs="$job" -size="${EACH_SIZE}M" -name=test --dedupe_percentage=0 -group_reporting -randseed="$i" | grep WRITE: | awk '{print $2}' | sed 's/bw=//g' | ../../nvm_tools-J/to_MiB_s)
                            ...
                            BW=$(bash ../../nvm_tools-J/"$TIMER" "$job" "${EACH_SIZE}"M "$dup_rate" "${BRANCHES[$STEP]}" "0" $SWAP_TIME "$swap_mem" "2M" | grep WRITE: | awk '{print $2}' | sed 's/bw=//g' | ../../nvm_tools-J/to_MiB_s)
                            cat /proc/fs/NOVA/pmem0/timing_stats > LOG.txt
                        fi
