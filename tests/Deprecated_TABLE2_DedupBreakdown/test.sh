    ...
    for fsize in "${FILE_SIZE[@]}"; do
        for job in "${NUM_JOBS[@]}"; do
            for branch in "${BRANCHES[@]}"; do
                ...
                # original-first
                VER1=$(bash ../../nvm_tools-J/setup_nova.sh "original" "1" | grep "COMMITID" | sed 's/COMMITID: //g')
                sudo mkdir -p /mnt/pmem0/origin-first
                BW=$(sudo fio -directory=/mnt/pmem0/origin-first -fallocate=none -direct=1 -iodepth 1 -rw=write -ioengine=sync -bs=2M -thread -numjobs="$job" -size="${EACH_SIZE}M" -name=test --dedupe_percentage=0 -group_reporting -randseed="$i" | grep WRITE: | awk '{print $2}' | sed 's/bw=//g' | ../../nvm_tools-J/to_MiB_s)
                ...
                VER2=$(bash ../../nvm_tools-J/setup_nova.sh "$branch" "1" | grep "COMMITID" | sed 's/COMMITID: //g')
                sudo mkdir -p /mnt/pmem0/first
                BW=$(sudo fio -directory=/mnt/pmem0/first -fallocate=none -direct=1 -iodepth 1 -rw=write -ioengine=sync -bs=2M -thread -numjobs="$job" -size="${EACH_SIZE}M" -name=test --dedupe_percentage=0 -group_reporting -randseed="$i" | grep WRITE: | awk '{print $2}' | sed 's/bw=//g' | ../../nvm_tools-J/to_MiB_s)
                ...
                BW=$(sudo fio -directory=/mnt/pmem0/second -fallocate=none -direct=1 -iodepth 1 -rw=write -ioengine=sync -bs=2M -thread -numjobs="$job" -size="${EACH_SIZE}M" -name=test --dedupe_percentage=0 -group_reporting -randseed="$i" | grep WRITE: | awk '{print $2}' | sed 's/bw=//g' | ../../nvm_tools-J/to_MiB_s)
